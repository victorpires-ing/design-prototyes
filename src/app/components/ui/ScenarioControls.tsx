...
import { 
  ...
  Ticket, ShieldAlert, ShoppingBag, BookOpen
} from "lucide-react";
...
export const ScenarioControls = ({ params, setParams }: any) => {
  ...
  // Opções de Variantes Simplificadas (Layout agora é automático por data)
  const variants = {
    times: ['unico', 'alguns', 'varios', 'muitos'],
    maps: ['nenhum', 'imagem', 'io']
  };

  // Helper para gerar datas visíveis no painel de bloqueio
  const generateDateRange = () => {
    if (!params.customStartDate) return [];
    const start = new Date(params.customStartDate + 'T00:00:00'); 
    const dates = [];
    
    // Se houver data fim, gera o range exato. Se não, gera os próximos 15 dias para teste.
              ...
              <div className="p-5 flex flex-col gap-6 min-h-screen pb-20">
                ...
                <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                  <div className="flex flex-col">
                    ...
                    <p className="text-xs text-gray-400">Layout reativo por datas</p>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-mono focus:border-[#ff6101] outline-none"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-500">Fim (Opcional)</label>
                      <input 
                        type="date" 
                  ...
                  <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                    <p className="text-[10px] text-blue-600 font-bold leading-tight">
                      INFO: {generatedDates.length === 1 
                        ? "Modo Data Única Ativo" 
                        : generatedDates.length <= 7 
                        ? `Modo 2-7 Dias Ativo (${generatedDates.length} dias)` 
                        : `Modo Calendário 8+ Ativo (${generatedDates.length} dias)`}
                    </p>
                  </div>
                </div>

                {/* --- GRUPO 2: REGRAS DE NEGÓCIO (LIMITES) --- */}
                ...
                <div className="space-y-3">
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                    <ShoppingBag size={12} /> Conteúdo Extras
                  </h3>
                  ...
                  <label className="flex items-center justify-between p-3 border border-gray-200 rounded-md cursor-pointer hover:border-gray-300 transition-all bg-white shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-gray-900">Ativar Produtos (Step 2)</span>
                    </div>
                    <div className={`w-10 h-5 rounded-full relative transition-colors ${params.hasCombos ? 'bg-[#ff6101]' : 'bg-gray-200'}`}>
                      <input type="checkbox" className="hidden" checked={!!params.hasCombos} onChange={(e) => setParams({ ...params, hasCombos: e.target.checked })} />
                      <div className={`absolute top-1 w-3 h-3 bg-white rounded-full shadow-sm transition-all ${params.hasCombos ? 'left-6' : 'left-1'}`} />
                    </div>
                  </label>
                </div>

                {/* --- GRUPO 4: OUTROS LAYOUTS --- */}
                <div className="space-y-4">
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Configurações de UI</h3>
                  <div className="grid grid-cols-2 gap-3">
                    {Object.entries(variants).map(([key, values]) => (
                      <div key={key} className="space-y-1.5">
                        <label className="text-[10px] font-bold text-gray-500 capitalize flex items-center gap-1">
                           {key === 'maps' ? <Map size={10}/> : <Calendar size={10}/>} {key}
                        </label>
                        <select 
                          value={params[key]} 
                ...
                <div className="space-y-3">
                   <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                    <AlertCircle size={12} /> Mock de Indisponibilidade
                  </h3>
                      ...
                      {generatedDates.map((date) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center py-2 rounded-md border transition-all ${isUnavailable ? 'bg-red-50 border-red-200 text-red-500' : 'bg-white border-gray-100 text-gray-600 hover:border-gray-300'}`}
                          >
                            <span className="text-[7px] font-bold uppercase">{date.toLocaleDateString('pt-BR', { weekday: 'short' }).slice(0, 2)}</span>
