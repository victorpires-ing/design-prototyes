import type { ClipboardEvent as ReactClipboardEvent, Key, KeyboardEvent, ReactNode } from "react";
...
export interface InputTagsProps {
    ...
    /**
     * Additional keys that commit the current input as a tag.
     * Enter is always a split key. Use this to add e.g. `[",", ";", " "]`.
     * When provided, paste events are also split on these delimiters (plus newlines)
     * so a single paste can create multiple tags.
     */
    splitOn?: string[];
    /**
     * Customize how each tag is rendered. Receives the tag label and its index
     * in `value`. When omitted, the label is rendered as plain text.
     */
    renderTag?: (label: string, index: number) => ReactNode;
}
...
export const InputTags = ({
    ...
    const addTagsBatch = useCallback(
        (texts: string[]): boolean => {
            ...
            const seenLabels = new Set(entries.map((e) => e.label));
            const added: string[] = [];

            for (const text of texts) {
                const trimmed = text.trim();
                if (!trimmed) continue;
                if (!allowDuplicates && seenLabels.has(trimmed)) continue;
                if (maxTags && nextEntries.length >= maxTags) break;
                if (validate && !validate(trimmed)) continue;

                const entry: TagEntry = { id: nextId(), label: trimmed };
            ...
            onChange?.(nextEntries.map((e) => e.label));
            added.forEach((label) => onTagAdded?.(label));
            return true;
        },
        [entries, isControlled, allowDuplicates, maxTags, validate, onChange, onTagAdded],
    );
    ...
    const handleInputKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
        ...
        // Commit current input on any custom split key (Enter is handled below).
        if (splitOn && splitOn.length > 0 && splitOn.includes(event.key)) {
    ...
    const handleInputPaste = (event: ReactClipboardEvent<HTMLInputElement>) => {
        if (!splitOn || splitOn.length === 0) return;
        const text = event.clipboardData?.getData("text") ?? "";
        if (!text) return;

        const escape = (s: string) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const chars = Array.from(new Set([...splitOn, "\n", "\r", "\t"])).map(escape).join("");
        const splitRegex = new RegExp(`[${chars}]+`);

        if (!splitRegex.test(text)) return; // no delimiters → fall through to normal paste

        event.preventDefault();
        const parts = text.split(splitRegex);
        const pending = inputValue.trim() ? [inputValue, ...parts] : parts;
        if (addTagsBatch(pending)) setInputValue("");
    };
                                        ...
                                        <TagList className="flex flex-wrap gap-1.5 focus:outline-hidden" items={entries}>
                                            {(item) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        {renderTag ? renderTag(item.label, idx) : item.label}
                                                    </Tag>
                                                );
                                            }}
                                        </TagList>
