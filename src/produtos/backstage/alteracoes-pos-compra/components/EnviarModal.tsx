...
interface EnviarModalProps {
    ...
    /** O que está sendo enviado — entra no título ("Enviar o link de pagamento"). */
    assunto: string;
...
/** Mesmo modal da bilheteria: pede o destino antes de enviar, mesmo quando já se conhece o
    e-mail do comprador — permite corrigir ou mandar para outra pessoa na hora. */
                ...
                <AriaDialog className="flex flex-col gap-5 rounded-xl bg-primary p-6 shadow-xl outline-hidden ring-1 ring-border-secondary">
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex flex-col gap-0.5">
                            <h2 className="text-lg font-semibold text-primary">{email ? `Enviar ${assunto} por e-mail` : `Enviar ${assunto} por WhatsApp`}</h2>
                            <p className="text-sm text-tertiary">{email ? "Confirme o e-mail de destino." : "Informe o WhatsApp com DDD."}</p>
                        </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        />
                        {tocado && !valido && <p className="text-sm text-error-primary">{email ? "Informe um e-mail válido." : "Informe um número com DDD."}</p>}
                    </div>
