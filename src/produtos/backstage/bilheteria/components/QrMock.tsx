 ...
 *
 * No protótipo o desenho não codifica nada: serve para a conversa de balcão
 * ("aponta a câmera aqui"), que é o que a tela precisa demonstrar.
 */
export function QrMock({ value, className }: { value: string; className?: string }) {
    ...
    return (
        <svg viewBox={`0 0 ${N * cell} ${N * cell}`} className={className} shapeRendering="crispEdges" role="img" aria-label={`QR code de ${value}`}>
