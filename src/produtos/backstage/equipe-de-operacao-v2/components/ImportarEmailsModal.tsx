...
function parseEmailsFromSheet(raw: string): ParseResult {
    ...
    if (idx !== -1) {
        const emails = lines
            .slice(1)
            .map((l) => (splitCells(l)[idx] ?? "").replace(/^["']|["']$/g, "").trim())
            .filter(Boolean);
...
export function ImportarEmailsModal({ isOpen, onClose, onConfirm }: Props) {
    ...
    const confirmar = () => {
        ...
        reader.onload = () => {
            ...
            if (!res.ok) {
                setErro(
                    res.reason === "no-email-column"
                        ? "Não encontramos uma coluna chamada email no arquivo."
                        : "O arquivo não tem nenhum contato.",
                );
                return;
            }
                ...
                <Dialog>
                    <div className="w-full max-w-md rounded-xl bg-primary p-6 shadow-xl ring-1 ring-border-secondary">
                        ...
                        {file && (
                            <div
                                className={cx(
                                    ...
                                    erro ? "ring-2 ring-error" : "ring-border-secondary",
                                )}
                            >
                                        ...
                                        <div className="flex min-w-0 flex-1 flex-col">
                                            ...
                                            <span className="text-xs text-tertiary">
                                                ...
                                                {erro && (
                                                    <>
                                                        {" "}
                                                        · <span className="font-medium text-error-primary">Falhou</span>
                                                    </>
                                                )}
                                            </span>
                                        </div>
                        ...
                        <div className="mt-6 flex justify-end gap-3">
                            <Button size="md" color="secondary" onClick={fechar}>
                                Cancelar
                            </Button>
                            <Button size="md" color="primary" isDisabled={!file || !!erro} onClick={confirmar}>
                                Importar e-mails
                            </Button>
                        </div>
                    </div>
