...
export interface EventConfig {
    ...
    /** Cadastro em três níveis, igual ao Backstage: grupo > ingresso > lote. */
    gruposIngresso: GrupoIngresso[];
    produtos: Produto[];
    datas: DataEvento[];
...
export const DEFAULT_CONFIG: EventConfig = {
    ...
    gruposIngresso: GRUPOS_INGRESSO,
    produtos: PRODUTOS,
    datas: DATAS,
...
/**
 * Converte a lista achatada dos links antigos na hierarquia atual, para um link
 * gerado antes da mudança continuar abrindo.
 */
function agruparIngressosAntigos(lista: Ingresso[]): GrupoIngresso[] {
    const grupos: GrupoIngresso[] = [];
    for (const item of lista) {
        const nomeGrupo = item.grupo || "Sem grupo";
        let grupo = grupos.find((g) => g.nome === nomeGrupo);
        if (!grupo) {
            grupo = { id: `g-${grupos.length}`, nome: nomeGrupo, acesso: "", ingressos: [] };
            grupos.push(grupo);
        }
        let tipo = grupo.ingressos.find((t) => t.nome === item.nome);
        if (!tipo) {
            tipo = { id: `t-${grupo.id}-${grupo.ingressos.length}`, nome: item.nome, descricao: item.descricao, ativo: true, lotes: [] };
            grupo.ingressos.push(tipo);
        }
        tipo.lotes.push({ id: item.id, nome: item.lote || "Lote único", preco: item.preco ?? 0, ativo: true });
    }
    return grupos;
}

/** Itens compráveis do evento — o fluxo de venda usa isto, nunca a hierarquia. */
export const ingressosDoEvento = (cfg: Pick<EventConfig, "gruposIngresso">) => achatarGrupos(cfg.gruposIngresso);
...
export function decodeConfig(param: string): EventConfig | null {
    try {
        ...
        return {
            ...
            gruposIngresso: Array.isArray(obj.gruposIngresso)
                ? obj.gruposIngresso
                : // Link antigo trazia a lista achatada: reagrupa pelo nome do grupo.
                  agruparIngressosAntigos(Array.isArray(obj.ingressos) ? obj.ingressos : []),
...
export function buildShareUrl(cfg: EventConfig): string {
    ...
    return `${base}/totem/event?cfg=${encodeURIComponent(encodeConfig(cfg))}`;
}
