...
import { AlertTriangle, CheckCircle, ChevronDown, Clock, ChevronRight, FileCheck02, InfoCircle, Minus, Package, Phone01, Plus, QrCode01, Send01, Tag01, Ticket01, Trash01, XClose } from "@untitledui/icons";
import { AnimatePresence, motion } from "motion/react";
import { Dialog, Modal, ModalOverlay } from "../../components/TotemModal";
...
import { TotemLayout, accentVars, useTotemPortal } from "../../components/TotemLayout";
import { LoginModal } from "../../components/LoginModal";
import { EtapaEntrega, type Entrega } from "../components/EtapaEntrega";
import { EtapaPagamento, TAXA, type Pagamento } from "../components/EtapaPagamento";
import { ProcessandoPagamento } from "../components/ProcessandoPagamento";
...
import { DEFAULT_CONFIG, decodeConfig, ingressosDoEvento, resolverLinkCurto, type EventConfig } from "../data/config";
...
export function SelecaoEAtribuicao() {
    ...
    /** Gaveta do resumo: portada para a tela do totem, não para a janela. */
    const portalTotem = useTotemPortal();
    ...
    const itemById = useMemo(() => {
        ...
        for (const i of ingressosDoEvento(config)) map.set(i.id, { id: i.id, nome: i.nome, grupo: i.grupo, lote: i.lote, descricao: i.descricao, preco: i.preco, imagem: i.imagem });
        for (const p of config.produtos) map.set(p.id, { id: p.id, nome: p.nome, preco: p.preco, imagem: p.imagem });
        return map;
    }, [config]);

    // Abas de combo fixo, agrupadas pelo rótulo configurável.
    ...
    // O que aparece é o que existe: cada aba é consequência do catálogo, não de um interruptor.
    const temDinamicos = config.combosDinamicos.length > 0;
    const datasVenda = config.datas;
    ...
    const [etapa, setEtapa] = useState<"selecao" | "produtos" | "atribuicao" | "entrega" | "pagamento">("selecao");
    /* Duas últimas perguntas do totem, depois de saber para quem é cada ingresso. */
    const [entrega, setEntrega] = useState<Entrega>("digital");
    const [pagamento, setPagamento] = useState<Pagamento | null>(null);
    const [parcelas, setParcelas] = useState(1);
    const [processando, setProcessando] = useState(false);
    /* `?ocioso=0` segura a tela ligada — para demonstrar o fluxo sem o totem zerar no meio. */
    const segundosOciosos = params.get("ocioso") !== null ? Math.max(0, Number(params.get("ocioso")) || 0) : 120;
    ...
    /** Recomeçar descarta a compra de alguém: pede confirmação antes. */
    ...
    // Carrinho vazio em qualquer etapa adiante da seleção → volta para o começo.
    useEffect(() => {
        if ((etapa === "atribuicao" || etapa === "entrega" || etapa === "pagamento") && unidades.length === 0) setEtapa("selecao");
    ...
    const finalizarPedido = () => {
        ...
        sp.set("entrega", entrega);
        if (pagamento) sp.set("pg", pagamento);
        const qs = sp.toString();
        navigate(`/totem/sucesso${qs ? `?${qs}` : ""}`);
    };

    /* Confirmação do pagamento: a maquininha e o Pix levam alguns segundos. */
    const pagar = () => {
        if (!pagamento) return;
        setProcessando(true);
        window.setTimeout(() => {
    ...
    const avancar = () => {
        ...
        if (etapa === "produtos") return irPara("atribuicao");
        if (etapa === "atribuicao") return podeFinalizar && irPara("entrega");
        if (etapa === "entrega") return irPara("pagamento");
        if (etapa === "pagamento") return pagar();
    };
    const irPara = (proxima: typeof etapa) => {
        setEtapa(proxima);
        window.scrollTo({ top: 0 });
    };

    const continuarDisabled =
        etapa === "selecao" ? totalItens === 0 : etapa === "atribuicao" ? !podeFinalizar : etapa === "pagamento" ? !pagamento : false;
    /* O rótulo diz para onde o botão leva — num totem ninguém lê o stepper. */
    const rotuloAvancar =
        etapa === "atribuicao" ? "Ir para a entrega" : etapa === "entrega" ? "Ir para o pagamento" : etapa === "pagamento" ? "Pagar" : "Continuar";

    /**
     * Zera tudo que é da pessoa que estava comprando. O totem é compartilhado:
     * carrinho, identificação e respostas não podem sobrar para o próximo.
     */
    const zerarSessao = () => {
        setCart({});
        setEtapa("selecao");
    ...
    /** Só faz sentido oferecer o recomeço quando existe algo a descartar. */
    const temSessao = totalItens > 0 || etapa !== "selecao" || usuario !== null;
    const voltarEtapa = () => {
        if (etapa === "pagamento") return irPara("entrega");
        if (etapa === "entrega") return irPara("atribuicao");
        if (etapa === "atribuicao") {
    ...
    const atribuicaoLayout = (
        <div className="mx-auto flex w-full max-w-[1446px] flex-col gap-6">
                        ...
                        {[
                            ...
                            return (
                                <button
                                    key={a.secao}
                                    type="button"
                                    onClick={() => navigate(`/totem/meia-entrada?secao=${a.secao}`)}
            ...
            <div className="flex w-full flex-col gap-4">{grupos.length > 0 && resumoCard}</div>
        </div>
    );
        ...
        <div className="flex flex-col gap-1">
            <span className="text-sm text-tertiary">
                ...
                {dataAtiva.abertura && dataAtiva.encerramento
                    ? `, das ${dataAtiva.abertura} às ${dataAtiva.encerramento}`
                    : dataAtiva.hora
                      ? `, ${dataAtiva.hora}`
                      : ""}
            </span>
    ...
    const umaDataLayout = (
        <div className="grid w-full grid-cols-1 gap-6">
            <div className="h-[260px] overflow-clip bg-secondary">
                {config.mapa ? (
            ...
            <div className="flex w-full flex-col gap-4">
                {cupomBlock}
                <div className="flex flex-col overflow-clip bg-primary md:rounded-2xl md:ring-1 md:ring-border-secondary">
                    <div className="flex flex-col px-4 pt-4 pb-4 md:px-5 md:pt-5">
                        {dataHeader}
                        {conteudo}
                    </div>
                    {totalItens > 0 && <div className="hidden">{totalBar}</div>}
                </div>
            </div>
        </div>
    );

    /* Só mostra endereço se for endereço mesmo: `usuario` guarda o nome, não o e-mail. */
    const emailEntrega = Object.values(atrib).find((a) => a?.email?.includes("@"))?.email;
    const linhasPedido = grupos.map(([, g]) => ({ nome: g.nome, sub: g.sub, qtd: g.qtd, preco: g.precoUnit }));

    const entregaLayout = (
        <div className="mx-auto flex w-full max-w-[1446px] flex-col gap-6">
            <EtapaEntrega valor={entrega} onChange={setEntrega} email={emailEntrega} />
        </div>
    );

    const pagamentoLayout = (
        <div className="mx-auto flex w-full max-w-[1446px] flex-col gap-6">
            <EtapaPagamento
                valor={pagamento}
    ...
    if (carregandoLink) {
        return (
            <TotemLayout title="Carregando…" usuario={usuario ?? undefined}>
    ...
    return (
        <TotemLayout
            ...
            onRecomecar={temSessao ? () => setConfirmarRecomeco(true) : undefined}
            onOcioso={zerarSessao}
            segundosOciosos={segundosOciosos}
        ...
        >
            {etapa === "pagamento" ? (
                ...
                <div className="grid w-full grid-cols-1 gap-6">
                    <div className="h-[320px] overflow-clip bg-secondary">
                        <img src={config.mapa} alt="Mapa do local" className="h-full w-full object-cover" />
                    </div>
                ...
                <div className="flex w-full flex-col gap-6">
                    <div className="flex w-full flex-col bg-primary p-4 md:rounded-2xl md:p-5 md:ring-1 md:ring-border-secondary">
                        {abas}
                        {conteudo}
                    </div>
            ...
            {/*
              Resumo fixo no rodapé, expansível. Portado para a tela do totem para ir de
              ponta a ponta do painel. Sem AnimatePresence: o `exit` do motion não assenta
              em todo renderizador e a barra ficava presa depois de esvaziar o carrinho.
            */}
            {totalItens > 0 && <div className="h-36" aria-hidden="true" />}
            {createPortal(
                <>
                    ...
                    {totalItens > 0 && (
                        <div
                            className="fixed inset-x-0 bottom-0 z-40 flex flex-col rounded-t-2xl bg-primary shadow-lg ring-1 ring-border-secondary"
                        ...
                        <AnimatePresence initial={false}>
                            {resumoAberto && (
                                ...
                                >
                                    <div className="flex max-h-[440px] flex-col gap-5 overflow-y-auto px-4 py-4">{resumoSecoes}</div>
                                </motion.div>
                            )}
                        </AnimatePresence>
            ...
            {processando && pagamento && (
                <ProcessandoPagamento metodo={pagamento} total={brl(totalValor * (1 + TAXA))} />
            )}

            <ConfirmarRecomecoModal
                isOpen={confirmarRecomeco}
                onClose={() => setConfirmarRecomeco(false)}
                onConfirmar={zerarSessao}
            />
...
/**
 * Recomeçar apaga a compra de quem está no totem. Aqui a confirmação vale mais
 * que um desfazer: o desfazer chega tarde demais quando quem perdeu o carrinho
 * já saiu da frente da tela.
 */
function ConfirmarRecomecoModal({ isOpen, onClose, onConfirmar }: { isOpen: boolean; onClose: () => void; onConfirmar: () => void }) {
    return (
        <ModalOverlay isOpen={isOpen} onOpenChange={(open) => !open && onClose()} isDismissable>
            <Modal className="sm:max-w-[420px]">
                <Dialog>
                    <div className="flex w-full flex-col gap-5 rounded-2xl bg-primary p-6">
                        ...
                        <div className="flex flex-col gap-3">
                            <Button size="lg" color="primary-destructive" onClick={onConfirmar}>
                                Sim, recomeçar
                            </Button>
...
/** Janela de funcionamento do dia e as sessões que acontecem dentro dela. */
function JanelaDoDia({ data }: { data: DataEvento }) {
    const sessoes = data.sessoes ?? [];
    if (!data.abertura && !data.encerramento && sessoes.length === 0) return null;
